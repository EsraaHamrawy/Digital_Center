...
const PurchaseLog = () => {
  const { pendingRequests, rejectedRequests, approvedRequests ,completeRequests} = useSelector(
  ...
  const rows = [...pendingRequests, ...rejectedRequests, ...approvedRequests,...completeRequests];
        ...
        return (
          <Box className={"cellActions"}>
            <Box className="more-infoClass">
              <LaunchIcon className="actionicon" onClick={()=>goToRequestDetails(params.row.id)} />
            </Box>
          </Box>
        );
      },
